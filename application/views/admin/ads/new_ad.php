<!--  ************************* Page Title Starts Here ************************** -->
<div class="page-title row no-margin">
	<h4>Manage Banner</h4>
	<ul>
		<li><a>Home <i class="fas fa-angle-double-right"></i></a></li>
		<li>Manage Banner</li>
	</ul>
</div> <!-- Page Title End -->


<!--  ************************* Manage Category Starts Here ************************** -->
<div id="vue-ad" class="row body-content">

	<?php $this->load->view('admin/ads/inc/position_1'); ?>

	<?php $this->load->view('admin/ads/inc/position_2'); ?>

	<?php $this->load->view('admin/ads/inc/position_3'); ?>

	<?php $this->load->view('admin/ads/inc/position_4'); ?>

	<?php $this->load->view('admin/ads/inc/position_5'); ?>

	<?php $this->load->view('admin/ads/inc/position_6'); ?>

	<?php $this->load->view('admin/ads/inc/footer'); ?>




</div><!-- Manage Category  End -->